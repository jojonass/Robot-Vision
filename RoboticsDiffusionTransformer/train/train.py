...
if "/home/e12434694/rdt_workspace/scripts" not in sys.path:
    sys.path.append("/home/e12434694/rdt_workspace/scripts")

# Import the class from the filename
from hdf5_maniskill_dataset import HDF5VLADataset
...
def train(args, logger):
    ...
    if args.load_from_hdf5:
        # Use your custom ManiSkill HDF5 loader
        logger.info("Using HDF5VLADataset (ManiSkill/Custom)")
        train_dataset = HDF5VLADataset(config_path=args.config_path)
        sample_dataset = HDF5VLADataset(config_path=args.config_path) # Or a subset if you have one
    else:
        # Fallback to the original RDT TFRecord loader
        logger.info("Using original VLAConsumerDataset (TFRecord)")
    ...
    # CRITICAL: Check that samples were actually found
    if len(train_dataset) == 0:
        raise ValueError("Dataset initialized with 0 samples. Verify paths in base.yaml.")                        
